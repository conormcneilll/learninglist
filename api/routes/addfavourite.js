...
let { learnlist_id, user_id } = req.body;
    ...
    connection.query(checkFavoriteQ, [learnlist_id, user_id], async (err, data) => {
        ...
        if (data.length > 0) {
            return res.json({ badstuff: { apimessage: "Learnlist is already favourited." } });
        } else {
            ...
            connection.query(favoriteQ, [learnlist_id, user_id], async (err, data) => {
                ...
                if (data.insertId) {
                    return res.json({ goodstuff: { apimessage: `Learnlist favourited.` } });
                } else {
                    return res.json({ badstuff: { apimessage: "Failed to favourite." } });
                }
            });
        }
    });
    ...
    console.error("Error in favouriting route: ", err.message);
